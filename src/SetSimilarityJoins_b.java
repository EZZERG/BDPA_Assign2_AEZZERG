...
public class SetSimilarityJoins_b extends Configured implements Tool {
	...
	public int run(String[] args) throws Exception {
		System.out.println(Arrays.toString(args));
		Job job = Job.getInstance(getConf(), "SetSimilarityJoins_b");

		job.setJarByClass(SetSimilarityJoins_b.class);
		...
		long counter = job.getCounters()
				.findCounter(MyCounter.nComparaisons_b).getValue();
		Path outFile = new Path("nComparaisons_b.txt");
		...
		public void reduce(Text key, Iterable<Text> values, Context context)
			...
			if (wordsL.size() > 1) {
				...
				for (String pair : pairs) {
					...
					context.getCounter(MyCounter.nComparaisons_b)
							.increment(1);
