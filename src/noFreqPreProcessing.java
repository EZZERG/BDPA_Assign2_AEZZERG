...
public class noFreqPreProcessing extends Configured implements Tool {

	private static final Logger LOG = Logger
			.getLogger(noFreqPreProcessing.class);
	...
	public static void main(String[] args) throws Exception {
		int res = ToolRunner.run(new noFreqPreProcessing(), args);
		System.exit(res);
	}

	public int run(String[] args) throws Exception {

		Job job = Job.getInstance(getConf(), "noFreqPreProcessing");
		...
		for (int i = 0; i < args.length; i += 1) {
			if ("-skip".equals(args[i])) {
				...
				LOG.info("Add file to the distributed cache: " + args[i]);
			}
		}
		...
		long counter = job.getCounters().findCounter(MyCounter.nlines)
				.getValue();
		Path outFile = new Path("nlines.txt");
	...
	public static class Map extends
		...
		private Set<String> SkipPattern = new HashSet<String>();
		...
		protected void setup(Mapper.Context context) throws IOException,
			...
			if (config.getBoolean("Preprocessing.skip.patterns", false)) {
				...
				SkipFileParser(localPaths[0]);
			}
		}

		private void SkipFileParser(URI patternsURI) {
		...
		public void map(LongWritable key, Text value, Context context)
			...
			for (String word : value.toString().split("\\s*\\b\\s*")) {
				...
				if (value.toString().length() == 0
						...
						|| SkipPattern.contains(word.toLowerCase())
	...
	public static class Reduce extends
		...
		public void reduce(LongWritable key, Iterable<Text> values,
			...
			reader = new BufferedReader(
					new FileReader(
							new File(
									"/home/cloudera/workspace/SETSIMILARITY/output/wordcount.txt")));
			String pattern;
			...
			StringBuilder wordscountSB = new StringBuilder();

			String firstprefix = "";
			for (String word : wordsHS) {
				wordscountSB.append(firstprefix);
				firstprefix = ", ";
				wordscountSB.append(word + "#" + wordcount.get(word));
			}

			java.util.List<String> wordscountL = Arrays
					.asList(wordscountSB.toString().split("\\s*,\\s*"));

			Collections.sort(wordscountL, new Comparator<String>() {
